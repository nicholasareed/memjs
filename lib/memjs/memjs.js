...
Client.prototype.get = function(key, callback) {
  if(callback === undefined) {
    ...
    return promisify(function(callback) {
      self.get(key, function(err, value, flags, cas) {
        callback(err, {value: value, flags: flags}, cas);
      });
    });
  }
...
Client.prototype.set = function(key, value, options, callback) {
  if(callback === undefined && typeof options !== 'function') {
    var self = this;
    return promisify(function(callback) { self.set(key, value, options, function(err, success, cas) { callback(err, success, cas); }); });
  }
  var logger = this.options.logger;
  var expires;
  ...
  var request = makeRequestBuffer(1, key, extras, value, this.seq, options.cas);
  this.perform(key, request, this.seq, function(err, response) {
    if (err) {
...
Client.prototype.add = function(key, value, options, callback) {
  if(callback === undefined && options !== 'function') {
    var self = this;
    return promisify(function(callback) { self.add(key, value, options, function(err, success, cas) { callback(err, success, cas); }); });
  }
  var logger = this.options.logger;
  var expires;
...
Client.prototype.increment = function(key, amount, options, callback) {
  if(callback === undefined && options !== 'function') {
    var self = this;
    return promisify(function(callback) {
      self.increment(key, amount, options, function(err, success, value, cas) {
        callback(err, {success: success, value: value}, cas);
      });
    });
  }
  var logger = this.options.logger;
  ...
  var request = makeRequestBuffer(5, key, extras, '', this.seq, cas);
  this.perform(key, request, this.seq, function(err, response) {
    if (err) {
      if (callback) { callback(err, null); }
      return;
    ...
    case 0:
      ...
      if (callback) { callback(null, true, bufInt, response.header.cas); }
      break;
    // TODO: failed cas
    default:
...
Client.prototype.decrement = function(key, amount, options, callback) {
  if(callback === undefined && options !== 'function') {
    var self = this;
    return promisify(function(callback) {
      self.decrement(key, amount, options, function(err, success, value, cas) {
        callback(err, {success: success, value: value}, cas);
      });
    });
  }
  ...
  expires = expires || this.options.expires;
  var extras = makeAmountInitialAndExpiration(amount, initial, expires);
  var request = makeRequestBuffer(6, key, extras, '', this.seq, cas);
  this.perform(key, request, this.seq, function(err, response) {
    if (err) {
      if (callback) { callback(err, null); }
      return;
...
//
//     callback(err, success)
Client.prototype.touch = function(key, options, callback) {
  if(callback === undefined) {
    var self = this;
    return promisify(function(callback) { self.touch(key, options, function(err, success, cas) { callback(err, success, cas); }); });
  }
  var logger = this.options.logger;
  var expires;
  var cas;
  if (typeof options === 'function') {
    // OLD: function(key, amount, callback, expires, initial)
    logger.log('MemJS TOUCH: using deprecated call - arguments have changed');
    return false;
  }
  ...
  var request = makeRequestBuffer(0x1C, key, extras, '', this.seq, cas);
  this.perform(key, request, this.seq, function(err, response) {
    if (err) {
      if (callback) { callback(err, null); }
      return;
    ...
    switch (response.header.status) {
    case 0:
      if (callback) { callback(null, true, response.header.cas); }
      break;
    case 1:
      if (callback) { callback(null, false); }
      break;
